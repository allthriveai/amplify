...
1. **Load config**: Read `.amplifyrc` via `loadConfig()`. Check that `studio.elevenlabsApiKey` exists.

2. **Check voice**: If no `studio.listenVoiceId` is configured:
   - Run `amplify listen --voices` to show available voices
   - Ask the user to pick one
   - Update `.amplifyrc` with the chosen `listenVoiceId` under the `studio` section
...
4. **Generate audio**: Run `amplify listen <note-filename>` (without .md extension works too)
...
- Missing ElevenLabs API key: Tell user to add `studio.elevenlabsApiKey` to `.amplifyrc`
