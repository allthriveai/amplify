...
- **The vault has three ownership layers.** `Sources/` is immutable: read it, cite it, never rewrite it. `Wiki/` is agent-owned: create and rewrite freely. `Journal/` is first-person and never gets ingested into the wiki. Anything writing to the vault has to respect that boundary
